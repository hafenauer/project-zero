...
def update_screen():
    ...
    draw_b.text((-1, start_y + 0 * row_gap + 8), "00.0", font=font_mono_readout_large, fill=0) # Temperature readout from SHT45 sensor
    draw_b.text((71, start_y + 0 * row_gap + 12), "°C", font=font_mono_medium, fill=0)
    
    tri_x = right_edge - 8
    tri_y = start_y + 0 * row_gap + 17
    draw_isosceles_triangle(draw_r, x=tri_x, y=tri_y, width=10, height=8, direction='down', fill=0) # Trend indicator. Triangle pointing down for cooler, up for warmer, not drawn if stable
    
    draw_b.text((71, start_y + 0 * row_gap + 24), "00.0", font=font_mono_medium, fill=0) # Temperature readout from OpenWeatherMap API for outside temp

    ### Humidity - inside and outside

    draw_r.text((-1, start_y + 1 * row_gap), "Humidity", font=font_mono_label, fill=0)

    draw_b.text((-1, start_y + 1 * row_gap + 8), "00.0", font=font_mono_readout_large, fill=0) # Humidity readout from SHT45 sensor
    draw_b.text((71, start_y + 1 * row_gap + 12), "%", font=font_mono_medium, fill=0)
    
    tri_x = right_edge - 8
    tri_y = start_y + 1 * row_gap + 17
    draw_isosceles_triangle(draw_r, x=tri_x, y=tri_y, width=10, height=8, direction='up', fill=0) # Trend indicator. Triangle pointing down for lower, up for higher, not drawn if stable
    
    draw_b.text((71, start_y + 1 * row_gap + 24), "00.0", font=font_mono_medium, fill=0) # Humidity readout from OpenWeatherMap API for outside humidity
    ...
    draw_b.text((24, start_y + 2 * row_gap + 12), "100", font=font_mono_readout_medium, fill=0) # VOC index readout from SGP41 sensor
    draw_b.text((-1, start_y + 2 * row_gap + 27), "NOx", font=font_mono_small, fill=0)
    draw_b.text((24, start_y + 2 * row_gap + 24), "001", font=font_mono_readout_medium, fill=0) # NOx index readout from SGP41 sensor

    draw_b.text((55, start_y + 2 * row_gap + 15), "PM", font=font_mono_small, fill=0)
    draw_b.text((80, start_y + 2 * row_gap + 12), "25", font=font_mono_medium, fill=0) # PM2.5 index readout from OpenWeatherMap API
    draw_b.text((55, start_y + 2 * row_gap + 27), "AQI", font=font_mono_small, fill=0)
    draw_b.text((80, start_y + 2 * row_gap + 24), "5", font=font_mono_medium, fill=0) # AQI index readout from OpenWeatherMap API
